...
class WXBot:
    ...
    def get_contact(self):
        ...
        while 1:
            url = self.base_uri + '/webwxgetcontact?pass_ticket=%s&skey=%s&r=%s&seq=%s' \
                                  % (self.pass_ticket, self.skey, int(time.time()),seq)
            ...
            for list1 in dic['MemberList']:
                self.member_list.append(list1)
            print dic['Seq']
            #seq = dic['Seq']
            if seq == 0:
                break
    ...
    def batch_get_group_members(self):
        ...
        group_members = {}
        encry_chat_room_id = {}
        for g_list in tmp_list:
            params = {
                'BaseRequest': self.base_request,
                "Count": len(g_list),
                "List": [{"UserName": group['UserName'], "EncryChatRoomId": ""} for group in g_list]
            }
            r = self.session.post(url, data=json.dumps(params))
            r.encoding = 'utf-8'
            dic = json.loads(r.text)
